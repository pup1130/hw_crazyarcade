...
public class Enemy extends Character implements Constant {
    ...
    private int[][] dir = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}};//방향을 나타내는 배열
    private int nx, ny;
    private int bomblength = 1;
    int speed = 6;
    private boolean[][] CanGo = new boolean[15][15];//갈 수 있는 블록
    ...
    private Queue<Point> queue = new LinkedList<>();//탐색을 위한 큐 선언
    ...
    public void SearchDir() {
        ...
        for (int j = 0; j < 4; j++) {
            for (int k = 1; k <= this.bomblength; k++) {
                if (CanGo[posx + dir[maxdir][0] + k * dir[j][0]][posy + dir[maxdir][1] + k * dir[j][1]]) {
                    maxmap--;
                }
